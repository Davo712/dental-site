...
(function () {
  ...
  /* ----------------------------------------------------------------------
     0. Ссылки при открытии файла с диска
     В адресах страниц расширения нет: на сервере это красивые /services
     и /contacts. Но протокол file:// имён без расширения не знает, поэтому
     при локальном открытии возвращаем .html обратно.
     ---------------------------------------------------------------------- */
  function initLocalLinks() {
    ...
    $$("a[href]").forEach(function (a) {
      ...
      a.setAttribute(
        "href",
        href === "./" ? "index.html" : href.replace(/^([^#?]+)/, "$1.html")
      );
    });
  }
